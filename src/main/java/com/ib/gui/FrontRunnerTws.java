...
import com.pmac.imap.ImapController;
...
import org.slf4j.LoggerFactory;
 ...
 public class FrontRunnerTws implements IConnectionHandler {
    
   final static Logger logger = LoggerFactory.getLogger(FrontRunnerTws.class);
   
   static { NewLookAndFeel.register(); }

    public static FrontRunnerTws INSTANCE;
    ...
    private MongoController m_mongo; // = new MongoController();
    private ImapController m_imap;
...
//    private Document m_advisorfirms;
    ...
    private String dbName = "frontrunner";
    
    static boolean verbose = false;
//    static boolean debug = false; // used in ImapController
...
//    static private Properties m_props = System.getProperties();
    ...
    public int m_connection_failures = 0;
...
//    }
    ...
    public static void main(String[] args) {
        FrontRunnerTws appMainThread = new FrontRunnerTws( new DefaultConnectionConfiguration() );
//        appMainThread
        start(appMainThread);
    }
	
    public static void start( FrontRunnerTws apiDemo ) {
        INSTANCE = apiDemo;
        INSTANCE.run();
    }

    public FrontRunnerTws( IConnectionConfiguration connectionConfig ) {
    ...
    public ApiController twsController() {
    ...
    public MongoController mongoController() {
        if ( m_mongo == null ) {
            m_mongo = new MongoController();
            m_mongo.init( m_acctList.get(0));
        }
        return m_mongo;
    }
    
    public ImapController imapController() {
        if ( m_imap == null ) {
            m_imap = new ImapController();
            m_imap.init();
        }
        return m_imap;
    }
    ...
    private void run() {
        try {
            ...
            twsController().connect( "127.0.0.1", TWS_PORT, 0, m_connectionConfiguration.getDefaultConnectOptions() != null ? "" : null );
...
//            }).start();
    ...
    public Position findPosition(Contract con) {
        return this.m_acctInfoPanel.findPosition(con);
    }
...
//         
//        logger.info("Position from TWS {}, Account {}, description, {}", pos, account, pos.contract().description());
//
//        MongoCursor<Document> itr = database.getCollection("recommendations").find(eq("positions._id", localSymbol)).iterator();
////        MongoCursor<Document> itr = database.getCollection("recommendations").find(eq("_id", localSymbol)).iterator();
//        if (itr.hasNext()) {
//            Document rec = itr.next();
//
//            logger.info("Recomendation {}, Account {}", rec, account);
//            
...
//                                
//                                 twsController().placeOrModifyOrder(con, order, null);
//                                                                
...
//    }
    ...
    public void connected() {
        ...
        m_connection_failures = 0; // reset

        twsController().reqCurrentTime((long time) -> {
            show( "Server date/time is " + Formats.fmtDate(time * 1000) );
        });

        twsController().reqBulletins(true, (int msgId, NewsType newsType, String message, String exchange) -> {
    ...
    @Override 
    public void updatePosition(Position pos) {
        mongoController().updatePosition(pos);
    ...
    @Override public void message(int id, int errorCode, String errorMsg) {
        ...
        if ( (errorCode == 502) && (m_connection_failures < 3) ) {
            MongoController.DB_NAME = "frontrunnerpaper";
            twsController().connect( "127.0.0.1", TWS_PORT_PAPER, 0, m_connectionConfiguration.getDefaultConnectOptions() != null ? "" : null );
        }
    ...
    private class ConnectionPanel extends JPanel {
        ...
        protected void onConnect() {
            ...
            twsController().connect( m_host.getText(), port, clientId, m_connectOptionsTF.getText());
        }
    }
